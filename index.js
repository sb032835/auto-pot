    ...
    mod.hook('S_PLAYER_STAT_UPDATE', 10, e => {
		if (config.enabled && config.hp) {
			...
			for (let hp = 0; hp < hpPot.length; hp++) {
				useCombat = hpPot[hp][1].inCombat ? mod.game.me.inCombat : true; inContract = mod.game.contract.active ? true : false;
				if (!hpPot[hp][1].inCd && ((!isSlaying && nowHP <= hpPot[hp][1].use_at && useCombat) || (isSlaying && nowHP <= hpPot[hp][1].slay_at && mod.game.me.inCombat)) && hpPot[hp][1].amount > 0 && mod.game.me.alive && !mod.game.me.inBattleground && !inContract &&!mod.game.me.mounted) {
		...
		if (config.enabled && config.mp) {
			...
			for (let mp = 0; mp < mpPot.length; mp++) {
				useCombat = mpPot[mp][1].inCombat ? mod.game.me.inCombat : true; inContract = mod.game.contract.active ? true : false;
				if (!mpPot[mp][1].inCd && nowMP <= mpPot[mp][1].use_at && mpPot[mp][1].amount > 0 && mod.game.me.alive && useCombat && !mod.game.me.inBattleground && !inContract && !mod.game.me.mounted) {
